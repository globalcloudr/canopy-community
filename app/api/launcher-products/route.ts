 ...
 *
 * Returns the product keys that are launchable for a given workspace.
 * Used by the product shell's in-app switcher to show only products the
...
async function getLaunchableProducts(
  workspaceId: string
): Promise<LauncherProductKey[]> {
  ...
  for (const attempt of attempts) {
    ...
    for (const row of (data as EntitlementRow[] | null) ?? []) {
      if (isLauncherProductKey(row.product_key) && canLaunchProduct(row)) {
